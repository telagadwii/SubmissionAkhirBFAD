...
st.markdown("Dashboard ini menampilkan hasil analisis penyewaan sepeda berdasarkan kondisi cuaca dan tren jam sibuk pada hari kerja.")
...
# ==============================
st.header("1. Dampak Cuaca terhadap Total Penyewaan (2012)")

# Persiapan Data
df_day2012 = df_day[df_day['dteday'].dt.year == 2012].copy()
weather_2012 = df_day2012.groupby('weathersit')['cnt'].mean().reset_index()
weather_2012 = weather_2012.sort_values(by='cnt', ascending=False)
...
ax1.set_title('Rata-rata Penyewaan Sepeda Berdasarkan Kondisi Cuaca (Tahun 2012)', fontsize=15)
...
with st.expander("Lihat Penjelasan"):
    st.write("Visualisasi ini menunjukkan bahwa rata-rata jumlah penyewaan sepeda tahun 2012 semakin menurun seiring dengan memburuknya cuaca. Cuaca cerah mendatangkan penyewa terbanyak, sedangkan cuaca hujan/bersalju drastis menurunkan angka penyewaan.")
...
# Persiapan Data
df_workingday = df_hour[df_hour['workingday'] == 'Workingday'].copy()
workingday_hourly = df_workingday.groupby('hr')['registered'].mean().reset_index()

# Menghitung metrik untuk ditampilkan
...
avg_morning = morning_rush['registered'].mean()
avg_evening = evening_rush['registered'].mean()
...
ax2.set_title('Tren Penyewaan Pengguna Terdaftar pada Hari Kerja (2011-2012)', fontsize=15)
...
ax3.set_title('Tren Penyewaan Pengguna Terdaftar per Jam Berdasarkan Suhu (Hari Kerja)', fontsize=15)
...
with st.expander("Lihat Penjelasan Lanjutan"):
    st.write("Hasil clustering sederhana (binning) memperlihatkan bahwa suhu **Cold** secara konsisten menekan volume penyewaan di bawah rata-rata sepanjang hari. Sebaliknya, saat suhu berada di kategori **Moderate** dan **Warm**, terjadi lonjakan penyewaan komuter yang signifikan, terutama pada puncak jam sibuk sore.")
